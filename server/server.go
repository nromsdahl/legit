...
// Config is the configuration for the legit service
type Config struct {
	KeySeed   string
	KeyFile   string
	AuthFile  string
...
// Server respresent a legit service
type Server struct {
	*cio.Logger
	config       *Config
	fingerprint  string
...
// NewServer creates and returns a new legit server
...
// Run is responsible for starting the legit service.
